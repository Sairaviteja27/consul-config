...
# ==== HARD-CODED CONFIG (matches your style) ====
NAMESPACE="objs-fed"
SERVICE_NAME="backend"
TIMEOUT_S="1"                          # requestTimeout to apply
RATE="60"
DURATION="60s"
...
YAML_FILE="timeouts.yaml"              # ServiceRouter with requestTimeout
...
# Parse Vegeta text report for totals, success ratio, status codes, bytes
parse_text_report() {
  local txt="$1"
  # Requests line
  ...
  # Bytes In (optional, not used in CSV but available)
  local bytes_in_total bytes_in_mean
  read -r bytes_in_total bytes_in_mean < <(grep -E '^Bytes In' "$txt" \
    | awk -F'[][]' '{print $3}' \
    | awk -F',' '{gsub(/ /,""); print $1, $2}')

  # Status codes → sum 2xx, count 504, sum others
  local status_line
  ...
  if [ -n "$status_line" ]; then
    while read -r pair; do
      ...
      if [[ -n "$code" && -n "$count" ]]; then
        parsed_total=$((parsed_total + count))
        if (( code >= 200 && code < 300 )); then
          success_cnt=$((success_cnt + count))
        fi
        if (( code == 504 )); then
          timeout_cnt=$((timeout_cnt + count))
        fi
      fi
    done < <(echo "$status_line" | grep -oE '[0-9]+:[0-9]+')
  fi

  # Prefer total from "Requests" line
  echo "$total" "$rate" "$throughput" "$success_ratio" "$success_cnt" "$timeout_cnt"
}

# Compute JSON-derived KPIs that text can’t provide
summarize_from_json() {
  local json="$1"
  local timeout_ms
  timeout_ms=$(awk -v t="$TIMEOUT_S" 'BEGIN{printf "%.6f", t*1000.0}')

  # Timeout Accuracy (only for .code==504 or error contains timeout/deadline)
  ...
  # Tail latency for successes only
...
for DELAY in "${DELAY_S_LIST[@]}"; do
  ...
  # Errors = TOTAL - SUCCESS_CNT (prefer counts from Status Codes; fallback to Success ratio if needed)
  ERRORS=$(( TOTAL - SUCCESS_CNT ))
  if [ "$SUCCESS_CNT" -eq 0 ]; then
    # fallback via ratio if status line missing
