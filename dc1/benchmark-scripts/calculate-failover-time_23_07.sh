...
ERROR_RATE=$(echo "$METRICS_JSON" | jq -r '[.errors[]] | join(", ") // "None"')
...
if [[ ! -f "$SUMMARY_FILE" ]]; then
  echo "Batch,SwitchTime,ErrorRate,MeanLatency(ms),P95(ms),P99(ms),Status200,Status503" > "$SUMMARY_FILE"
fi

BATCH_ID=$(($(grep -c '^' "$SUMMARY_FILE")))
echo "$BATCH_ID,$DIFF,\"$ERROR_RATE\",$MEAN_MS,$P95_MS,$P99_MS,$STATUS_200,$STATUS_503" >> "$SUMMARY_FILE"
